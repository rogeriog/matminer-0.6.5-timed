...
class TimeTable():
    ...
    def updateTable(self, total_time, column_names, row_label):
        self.timetable=pd.concat([self.timetable, pd.DataFrame(total_time,columns=column_names,index=row_label)])
...
class BaseFeaturizer(BaseEstimator, TransformerMixin, ABC):
    ...
    def featurize_dataframe(self, df, col_id, ignore_errors=False,
                            ...
                            multiindex=False, pbar=True, timetable=None):
        """
        ...
        pooled_results = self.featurize_many(df[col_id].values,
        ...
        if inplace:
            ...
            if timetable is not None:
                return (new[df.columns.tolist() + res.columns.tolist()], timetables)
            else:
    ...
    def featurize_many(self, entries, ignore_errors=False, return_errors=False,
        ...
        if self.n_jobs == 1:
            return [self.featurize_wrapper(x, ignore_errors=ignore_errors,
                                           return_errors=return_errors, timetable=timetable) for x in entries]
        else:
            ...
            with Pool(self.n_jobs) as p:
                func = partial(self.featurize_wrapper,
                               return_errors=return_errors,
                               ignore_errors=ignore_errors, timetable=timetable)
                pooled_results=p.map(func, entries, chunksize=self.chunksize)
                return pooled_results
    def featurize_wrapper(self, x, return_errors=False, ignore_errors=False, timetable=None):
        """
